...
sys.path.append(os.path.abspath("/LinuxShowDirector/prod/shaders"))
import shaders
import colors
...
class display:
	...
	def __init__(self):
		...
		self.path = "/LinuxShowDirector/prod"
	def init(self):
	...
	def init_shader(self):
		...
		self.shaders["stop_a"] = shaders.stop_a.stop_a_shader(self)
		self.shaders["next"] = shaders.next.next_shader(self)
		self.shaders["clear"] = shaders.clear.clear_shader(self)
	def write(self):
		result = ""
	...
	def add_color(self,text):
		return ["\033[" + str(self.colors[self.color][0]) + ";" + str(self.colors[self.background_color][1]) + "m", text, "\033[0m"]
