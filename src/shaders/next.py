...
class NextShader(AnimationShader):
    def __init__(self, display):
        def shader(self, x, y, progress):
            ...
            for y in range_y:
                for x in range_x:
                    if self.display.translate_x(-1.3 * (self.display.columns / self.display.lines) * progress) <= x <= self.display.translate_x(
                            1.3 * (self.display.columns / self.display.lines) * progress):
                        if self.display.translate_y(
                                1.2 * progress) <= y <= self.display.translate_y(-1.2 * progress):
                            self.display._set(
